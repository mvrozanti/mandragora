...
8. **Holder metadata is advisory.** `gpu.lock.holder` exists only for `status`/`GpuBusy`; the `flock` on `gpu.lock` is the real mutex. Failing to write, read, or clear the holder file never denies or wedges the lock — it is logged and the lock proceeds. This matters across UIDs: `/dev/shm/gpu-lock` is sticky, so a stale holder file from a crashed root acquirer (`emotion-web`) cannot be replaced by a user-bot acquirer; the bot still gets the lock, and `status` just under-reports until the next successful write. `gpu-status` (running as `m`) opportunistically clears dead holders it can.
