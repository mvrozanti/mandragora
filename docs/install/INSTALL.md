...
```bash
sudo bash docs/install/format-drive.sh /dev/nvme0n1
```

### 3. Mount subvolumes
```bash
sudo bash docs/install/mount-install.sh
```
...
```bash
sudo bash docs/install/bootstrap-age-key.sh
```
...
### 6. Install NixOS
```bash
sudo bash docs/install/install.sh
```
...
| ----------------------------------- | --------------------------------------------------------------------------------- |
| `docs/install/format-drive.sh`           | Creates ESP, swap, Btrfs pool. Inside the pool: `root-blank`, `root-active`, `nix`, `persistent` subvolumes. |
| `docs/install/mount-install.sh`          | Mounts subvolumes at `/mnt`, `/mnt/nix`, `/mnt/persistent`, etc., for the install. |
| `docs/install/bootstrap-age-key.sh`      | Generates the age keypair, writes the private key to the persistent path, derives the public key, encrypts `secrets/secrets.yaml` against it. Prompts for the login password. |
| `docs/install/install.sh`                | Runs `nixos-install --flake .#mandragora-desktop` against the mounted target.     |
...
- Run `nix/modules/audits/strays.sh` once manually to confirm no unexpected
  state.
