#!/usr/bin/env bash
# Deploy ofin.mvr.ac — sync source, build container, bring stack up.
# Idempotent. No third-party secrets; data flows from local PDF ingest.
#
# Env overrides:
#   REMOTE        ssh target              default opc@mandragora-vps
#   REMOTE_DIR    slot on VPS             default /home/opc/ofin
#   LOCAL_REPO    app source              default ~/Projects/ofin
#   SEED_DB       1 to pg_dump local + restore on VPS              default 0

set -euo pipefail

REMOTE="${REMOTE:-opc@mandragora-vps}"
REMOTE_DIR="${REMOTE_DIR:-/home/opc/ofin}"
LOCAL_REPO="${LOCAL_REPO:-$HOME/Projects/ofin}"
COMPOSE_SRC="$(cd "$(dirname "$0")" && pwd)/docker-compose.yml"
SEED_DB="${SEED_DB:-0}"
LOCAL_PG_HOST="${LOCAL_PG_HOST:-/tmp}"
LOCAL_PG_PORT="${LOCAL_PG_PORT:-5433}"
LOCAL_PG_USER="${LOCAL_PG_USER:-ofin}"
LOCAL_PG_DB="${LOCAL_PG_DB:-ofin}"

if [[ ! -d "$LOCAL_REPO/src/ofin" ]]; then
  echo "ERR: $LOCAL_REPO/src/ofin not found (set LOCAL_REPO)" >&2
  exit 1
fi
if [[ ! -f "$COMPOSE_SRC" ]]; then
  echo "ERR: docker-compose.yml not found next to deploy.sh" >&2
  exit 1
fi

if [[ -z "${OFIN_DB_PASSWORD:-}" ]]; then
  existing="$(ssh "$REMOTE" "grep -E '^OFIN_DB_PASSWORD=' $REMOTE_DIR/.env 2>/dev/null | cut -d= -f2-" || true)"
  if [[ -n "$existing" ]]; then
    OFIN_DB_PASSWORD="$existing"
    echo "→ reusing OFIN_DB_PASSWORD from existing $REMOTE_DIR/.env"
  else
    OFIN_DB_PASSWORD="$(head -c 32 /dev/urandom | od -An -tx1 | tr -d ' \n')"
    echo "→ generated fresh OFIN_DB_PASSWORD (only on first deploy)"
  fi
fi

DB_SUBDIR="${DB_SUBDIR:-db-pdf}"

echo "→ ensuring remote slot $REMOTE:$REMOTE_DIR exists"
ssh "$REMOTE" "mkdir -p $REMOTE_DIR/src $REMOTE_DIR/$DB_SUBDIR $REMOTE_DIR/uploads"

echo "→ rsyncing app source to $REMOTE:$REMOTE_DIR/src/"
rsync -av --delete \
  --exclude='__pycache__/' --exclude='*.pyc' \
  --exclude='.venv/' --exclude='.git/' \
  "$LOCAL_REPO/" "$REMOTE:$REMOTE_DIR/src/"

echo "→ syncing compose.yml"
rsync -av "$COMPOSE_SRC" "$REMOTE:$REMOTE_DIR/docker-compose.yml"

echo "→ writing $REMOTE:$REMOTE_DIR/.env (mode 0600)"
ssh "$REMOTE" "umask 077 && cat > $REMOTE_DIR/.env" <<EOF
MVR_AC=mvr.ac
OFIN_IMAGE=ofin:latest
OFIN_DB_DIR=$REMOTE_DIR/$DB_SUBDIR
OFIN_DB_PASSWORD=$OFIN_DB_PASSWORD
OFIN_DEFAULT_USER=m
OFIN_UPLOAD_DIR=$REMOTE_DIR/uploads
EOF

echo "→ building image ofin:latest on VPS"
ssh "$REMOTE" "cd $REMOTE_DIR/src && docker build -t ofin:latest ."

echo "→ docker compose up -d"
ssh "$REMOTE" "cd $REMOTE_DIR && docker compose up -d"

echo "→ waiting for db + healthz"
sleep 8

if [[ "$SEED_DB" == "1" ]]; then
  DUMP=/tmp/ofin-seed-$(date +%s).sql
  echo "→ pg_dump local db → $DUMP"
  pg_dump -h "$LOCAL_PG_HOST" -p "$LOCAL_PG_PORT" -U "$LOCAL_PG_USER" \
    --no-owner --no-privileges --clean --if-exists "$LOCAL_PG_DB" > "$DUMP"
  echo "→ shipping $(wc -l < "$DUMP") lines to $REMOTE:$REMOTE_DIR/seed.sql"
  scp "$DUMP" "$REMOTE:$REMOTE_DIR/seed.sql"
  rm -f "$DUMP"
  echo "→ restoring inside ofin-db container"
  ssh "$REMOTE" "docker cp $REMOTE_DIR/seed.sql ofin-db:/tmp/seed.sql && \
    docker exec ofin-db psql -U ofin -d ofin -f /tmp/seed.sql >/tmp/restore.log 2>&1 && \
    tail -5 /tmp/restore.log"
fi

ssh "$REMOTE" "docker exec ofin wget -qO- http://localhost:8080/healthz || echo '(healthz check failed)'"

echo "→ done. visit https://ofin.mvr.ac (authelia-gated)."
echo "   logs:    ssh $REMOTE 'docker logs -f ofin'"
echo "   status:  ssh $REMOTE 'docker ps --filter name=ofin'"
echo "   seed:    SEED_DB=1 bash $0   (pg_dump local + restore on VPS)"
