...
function onKeyEvent(key, isPress)
    ...
    if graph[key.name] then
        thread(cascadeKeys, key.name, 0, numJumps, {})
    end
end
...
function isVisited(visited, keyName)
    for i = 1,#visited do
        if visited[i] == keyName then
            return true
        end
    end
    return false
...
function cascadeKeys(keyName, level, maxLevel, visited)
    ...
    wait(delay)
    for _, nextKey in ipairs(neighbors) do
        if not isVisited(visited, nextKey) then
            cascadeKeys(nextKey, level+1, maxLevel, visited)
        end
    end
end
