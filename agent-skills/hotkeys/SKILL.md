...
- About to edit any of: `.config/hypr/hyprland.conf`, `.config/aerc/binds.conf`, `.config/nvim/map.vim`, `.config/zsh/zshrc.zsh`, `.config/tmux/tmux.conf`, `.config/mpv/input.conf`, `modules/user/{yazi,waybar,zx-dirs}.nix`.
...
- `scope`: `hyprland | aerc | nvim | zsh | tmux | yazi | mpv | waybar`
...
| Terminal apps    | `CTRL`, `ALT`            | nvim/tmux/aerc/zsh/yazi — same chord = related action where possible |
...
`hjkl` means left/down/up/right *everywhere* — Hyprland focus, tmux pane, yazi nav, vim, aerc message list. When proposing a movement-like action, default to hjkl. When proposing a non-movement action, **avoid** hjkl unless you're extending the family (e.g. `SHIFT+H/J/K/L` for "move thing in that direction").
...
- `z<letter>` — directory shortcut family. **Single source of truth: `modules/user/zx-dirs.nix`**. Consumed by both `yazi` (as `g<letter>`) and `zsh` (as `z<letter>` shell aliases). Never add a `z<letter>` directly in `yazi.nix` or `zshrc.zsh` — edit `zx-dirs.nix`.
- `o<letter>` — sort-order in yazi (`os`/`oS`/`om`/`oM`/`oc`/`oC`).
- `y<letter>` — yank variants in yazi (`P`/`N`/`B` for path/name/bytes).
...
awk -F'\t' '$1=="yazi" && $4 ~ /^o/' /tmp/hotkeys.tsv
```
...
1. **Single source of truth for `z<letter>`** — directory shortcuts go in `modules/user/zx-dirs.nix`. Never add `z<letter>` bindings directly to yazi.nix or zshrc.zsh.
