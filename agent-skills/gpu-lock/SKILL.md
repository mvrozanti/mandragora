...
4. **Ollama callers acquire the lock themselves; no proxy.** llm-via-telegram, local-ai-mcp-server, and meme wrap every `/api/chat`|`/api/generate` POST in `gpu_lock.acquire_async(...)` plus `evict_model(..., keep_alive=0)` in the `finally`. Any new Ollama client must do the same — there is no Layer-2 proxy and none is planned. Bare `crush` invocations against `127.0.0.1:11434` from one-off shells are the only remaining bypass; treat that as user-supervised, not a contract violator. **Pinning `keep_alive=-1` is forbidden** — a SIGKILL/oomd skips your `finally` and leaks the entire model into VRAM until next reboot. Use a bounded value (e.g. `"10m"`) so the leak self-clears.
5. **Unmanaged GPU users defer you too.** `acquire`/`gpu-lock run` also refuse when a non-cooperative process (a Steam/Proton game) holds ≥ `GPU_LOCK_FOREIGN_VRAM_MIB` (default 1000) of VRAM — treat the resulting `GpuBusy` exactly like a held lock. Set the env to `0` to disable.

## Inspection

- `gpu-lock status [--json]` — current holder (pid, name, held_for, expected_remaining); `--json` emits `{"holder": {...}|null, "unmanaged": {...}|null}` for any non-cooperative GPU process
