...
smp(){ rsync -r --progress --checksum --files-from=<(comm -13 <(ls -1 ~/Musik/ | cut -c 3- | sort) <(ls -1 "$HOME/phone/SD card/Music/" | cut -c 3- | sort) | grep '\.mp3$') ~/Musik/ "$HOME/phone/SD card/Music/" }
