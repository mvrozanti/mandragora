...
alias cfc='v ~/mandragora/.dottyrc.json'
...
alias cfV='v ~/.config/nvim'
alias cfx='sudo nvim -u NORC ~/.xinitrc'
alias cfz='v ~/.zshrc'
...
alias rp='realpath -z'
wis(){ realpath $(whereis "$1" | cut -d':' -f2 | cut -d' ' -f2) }
...
alias tfl='tf *.log'
alias cfK='nvim ~/.config/keyledsd.template.conf'
...
ca(){ replace -s "$(sa $@)" "`vipe<<<$(sa $@)`" -- ~/.bash_aliases }
...
co2ip(){ f=/tmp/co2i-$(date +%s).png; xclip -selection clipboard -t image/png -o > "$f" && echo -n "$f"; }
co2ipc(){ co2ip | c }
...
co2pinta(){ xclip -selection clipboard -t image/png -o > /tmp/img; pinta /tmp/img }
...
alias wdush.='wdu -sh .'
lf () { cd "$(command lf-ueberzug -print-last-dir "$@")" }
