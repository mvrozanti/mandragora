...
lnb(){ [[ $# != 2 ]] && return; le_fpath="$1"; le_dst="$2"; sudo ln -s `realpath $le_fpath` ~/.local/bin/`[[ -z "$le_dst" ]] && echo $le_fpath|cut -f 1 -d '.' || echo $2` }
...
alias lasagna='countdown "11*60" && for i in {1..4}; do sudo env -i beep -l 400; sleep 0.6; done'
