...
function lightUpKeys(keyName, level, maxLevel, visited)
    ...
    tries = 0
    repeat
        local random_index = math.random(1, #neighbors)
        nextKey = neighbors[random_index]
        tries = tries + 1
    until (not isVisited(visited, nextKey) or tries > 10)
    if nextKey == nil then
        return
    end
...
function lightUpKey(keyName, level, maxLevel)
    ...
    if key then
        if level == 0 then
            buffer[key] = fade(1, color, tocolor("black"))
        else
            buffer[key] = fade(fadeTime*(level+2)/11, color, tocolor("black"))
        end
    end
end
