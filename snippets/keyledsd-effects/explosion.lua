...
numLayers = tonumber(keyleds.config.numLayers) or 3
delay = tonumber(keyleds.config.delay) or 0.1
...
function onKeyEvent(key, isPress)
    ...
    if graph[key.name] then
        thread(lightUpKeys, key.name, numLayers)
    end
end
...
function lightUpKeys(keyName, layers)
    local allKeys = { [keyName] = true }
    local currentLayer = { keyName }
    
    for i = 1, layers do
        local nextLayer = {}
        for _, k in ipairs(currentLayer) do
            if graph[k] then
                for _, neighbor in ipairs(graph[k]) do
                    if not allKeys[neighbor] then
                        allKeys[neighbor] = true
                        table.insert(nextLayer, neighbor)
                        lightUpKey(neighbor, i, layers)
                    end
                end
            end
        end
