...
**10. Worktree by Default + Mid-Switch Guard**
**Default to a `git worktree`** for any edit under `/etc/nixos/mandragora/`. Direct edits to the main tree are the *exception*, permitted only when the change is a single-file, sub-30-second edit you can stage and commit in one stroke. If in doubt, worktree. `pgrep -af "nixos-rebuild switch"` and `pgrep -af "mandragora-switch"` are necessary but **not sufficient** — they only catch a *running* rebuild, not another agent that is currently editing, staging, or about to switch. A clean `pgrep` is not proof of solitude, so it does not unlock direct-tree editing. The known failure mode this rule exists to prevent: a parallel `mandragora-switch`'s `git add -A` sweeping your untracked files into its commit (memory: `feedback_parallel_commit_staging_leak.md`). `mandragora-switch`'s flock + stability window protect against switch-vs-switch races but do **not** stop that index leak. Full protocol — including the worktree create/merge/cleanup recipe — in [`docs/worktrees.md`](docs/worktrees.md).
...
All CUDA / PyTorch / Ollama work must serialize through `gpu-lock`. PyTorch holders must call `torch.cuda.empty_cache()` before release. Ollama is not yet in the protocol — until the proxy ships, manually stop it for exclusive workloads. Full contract: invoke the `gpu-lock` skill (Claude/Gemini), read [`~/.ai-shared/rules/gpu-lock.md`](../../.ai-shared/rules/gpu-lock.md) (any agent), or [`docs/gpu.md`](docs/gpu.md) for the long-form rationale.
...
When migrating to a new local LLM (e.g., Model A -> Model B), the agent MUST update all following references:
